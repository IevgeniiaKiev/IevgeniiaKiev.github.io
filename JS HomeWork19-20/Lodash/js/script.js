...
var arraySkills = _.flatten(skills);

//uniq - Создает копию массива без дубликатов
var unique = _.uniq(arraySkills);

//sortBy - 	Возвращает массив сортированных по возрастанию элементов коллекции
var sorted = _.sortBy(unique);

console.log('Сортировка по скилам без повторов', sorted);

var sortFriendsNumber = _.sortBy(data,'friends.length');
var nameByFriends = _.map(sortFriendsNumber, 'name');

console.log('Сортировка по именам в зависимости от количества друзей', nameByFriends);

var friend = _.map(data, 'friends');
var arrayFriends = _.flatten(friend);
var friendName = _.map(arrayFriends, 'name');
var uniqueFriends = _.uniq(friendName);

console.log('Все друзья без повторов имен', uniqueFriends);
